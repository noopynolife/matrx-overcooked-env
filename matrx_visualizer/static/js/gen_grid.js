...
function draw(state, world_settings, new_messages, accessible_chatrooms, new_tick) {
	...
	// If the simulation is paused, show the start menu
	if (current_tick === 0 && latest_tick_processed === current_tick){
		show_start_menu();
	}else{
		hide_start_menu();
	}

	// show or hide the cover screen if that is necessary
	update_cover_screen();
    ...
    obj_ids.forEach(function(objID) {
		...
		// Overcooked EDIT
        if (obj.hasOwnProperty('class_inheritance') && obj['class_inheritance'].includes("ArtificialCookBrain")) {
			
			// Generate current task queue of the agent, do so every tick
			if (new_tick && obj.hasOwnProperty('queued_tasks')){
				task_queue = obj['queued_tasks'];
				repopulate_agent_tasks(task_queue);
			}
			

			// If we are a task based agent, generate all the task buttons
			if (obj['class_inheritance'].includes("TaskBasedCookBrain")){
				
				submenu = document.getElementById("play-based");
				if (submenu.style.display !== 'none'){submenu.style.display = 'none';}
				submenu = document.getElementById("goal-based");
				if (submenu.style.display !== 'none'){submenu.style.display = 'none';}				
				submenu = document.getElementById("task-based");
				if (submenu.style.display === 'none'){submenu.style.display = 'block';}
				
				// Generate the recipe related task buttons, but do so only once
				if (recipe_tasks.length === 0 && obj.hasOwnProperty('recipeBook')){
					recipe_tasks = obj['recipeBook']['recipe_dict'];
					
					gettables = obj['recipeBook']['simple_ingredients'];
					populate_group(gettables, 'get', 'get');	
					
					//mergables = obj['recipeBook']['merge_recipes'];
					//tool_recipes = obj['recipeBook']['tool_recipes'];
					//producables = unpack_tool_tasks(tool_recipes);
					//preps = mergables.concat(producables);
					//populate_group(preps, 'prepare', 'prepare');
					
					//deliverables = obj['recipeBook']['deliverables'];
					//populate_group(deliverables, 'serve', 'serve');
				}
				
				// Generate the move task buttons, but do so only once
				if (use_tasks.length === 0 && obj.hasOwnProperty('use_tasks')){
					use_tasks = obj['use_tasks'];
					populate_group(use_tasks, 'use', 'use');
				}
				
				// Generate the use task buttons, but do so only once
				if (move_tasks.length === 0 && obj.hasOwnProperty('move_tasks')){
					move_tasks = obj['move_tasks'];
					populate_group(move_tasks, 'move', 'move');
				}
			}
			
			// If we are a goal based agent, generate all the priority buttons
			if (obj['class_inheritance'].includes("GoalBasedCookBrain")){
				submenu = document.getElementById("task-based");
				if (submenu.style.display !== 'none'){submenu.style.display = 'none';}
				submenu = document.getElementById("play-based");
				if (submenu.style.display !== 'none'){submenu.style.display = 'none';}				
				...
				// Generate the move task buttons, but do so only once
				if (priority_tasks.length === 0 && obj.hasOwnProperty('priority_tasks')){
					priority_tasks = obj['priority_tasks'];
					populate_group(priority_tasks, 'priority', 'priority');
				}
				
				// Highlight the active priority each tick
				if (new_tick && obj.hasOwnProperty('active_priorities')){
					highlight_button_in_group('priority', obj['active_priorities'], added_text=' (active)');
				}
			}
			
						
			// If we are a play based agent, generate all the play buttons
			if (obj['class_inheritance'].includes("PlayBasedCookBrain")){
				submenu = document.getElementById("task-based");
				if (submenu.style.display !== 'none'){submenu.style.display = 'none';}
				submenu = document.getElementById("goal-based");
				if (submenu.style.display !== 'none'){submenu.style.display = 'none';}				
				...
				// Generate the move task buttons, but do so only once
				if (play_tasks.length === 0 && obj.hasOwnProperty('play_tasks')){
					play_tasks = obj['play_tasks'];
					populate_group(play_tasks, 'play', 'play');
				}
				
				// Highlight the active play each tick
				if (new_tick && obj.hasOwnProperty('active_plays')){
					highlight_button_in_group('play', obj['active_plays'], added_text=' (active)');
				}
			}
        }
		// Do goal related visualization tasks
		else if (new_tick && obj.hasOwnProperty('is_delivery') && obj.hasOwnProperty('goals')) {
			//Update the timer
			update_timer(obj['endtick'], world_settings);
			
			//Update the scoreboard
			update_scoreboard(obj['delivered'].length);

			//Display the goals
			goals = obj['goals']; 
			active_goals = [];
			for (var i = 0; i < goals.length; i++){	
				if (goals[i]['appears_at'] <= current_tick && current_tick < goals[i]['max_nr_ticks']){
					active_goals.push(goals[i]);
				}
			}
			repopulate_goals(active_goals, world_settings['nr_ticks']);
			
			//Update welcome and goodbye texts
			if (current_tick != 0){
				goodbye = document.getElementById("goodbye");
				goodbye.innerHTML = obj['goodbye_message'];
			}else{
				welcome = document.getElementById("welcome");
				welcome.innerHTML = obj['welcome_message'];
			}
			
			//Display the pop-up at the end of the simulation
			if (obj['simulation_completed']){
				show_end_simulation(obj['delivered'].length);
			}
		}
	...
	// hide the loading screen when rendering is complete
	hide_loading_screen();
}
